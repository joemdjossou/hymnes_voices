...
def main():
    print("=== h469 Channel Trimming (Remove first 55 seconds) ===")
    ...
    # Verify the result
    print(f"\n=== Verification ===")
    updated_files = sorted(list(h469_dir.glob("channel_*.mp3")))
    print(f"Channel files in h469: {[f.name for f in updated_files]}")
    ...
    if success_count > 0:
        print("\n🎉 h469 trimming completed successfully!")
        print("All channels now have the first 55 seconds removed!")
    else:
